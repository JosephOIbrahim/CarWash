...
1. **Plan before execute.** Current plan lives at `state/plan.md`. No agent runs without a plan entry.
...
1. **No new dependencies without a decision.** `npm/pip install`, new services, new API keys → record in `state/decisions.md` first.
...
1. **Human-only decision** — anything you can’t justify from `state/decisions.md` or the plan.
...
- Externalize reasoning to `state/decisions.md`. Don’t hold the plan only in context.
...
If `EXIT_CRITERIA` is not provided, the planner generates a draft set as its first act and records them in `state/decisions.md` for confirmation. **No work begins until exit criteria exist.**
